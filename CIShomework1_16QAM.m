...
% matched filtering
%  F1=fft(Txdata15dB);
% F=conj(fft(sin(2*pi*(0:0.001:1)),length(F1)));
% Txdata15dB=abs(ifft(F1.*F'));
...
%-------After demodulation we can pass the signal through a neural net
%instead of using a raised cosine filter
...
N_sample;
ratio=0.5;
Trainsize=round(length(O_data))*ratio;
for i=1:Trainsize
    X_dataA(i,1:N_sample)=RxdataA5((64*(i)-64)+(1:64));
    X_dataB(i,1:N_sample)=RxdataB5((64*(i)-64)+(1:64));
end
for i=1:length(O_data)
    X_dataAC(i,1:N_sample)=RxdataA5((64*i-64)+(1:64));
    X_dataBC(i,1:N_sample)=RxdataB5((64*i-64)+(1:64));
end
for i=1:length(O_data)
    X_dataAC15(i,1:N_sample)=RxdataA15((64*i-64)+(1:64));
    X_dataBC15(i,1:N_sample)=RxdataB15((64*i-64)+(1:64));
end
for i=1:Trainsize
    X_dataA(i,1:N_sample)=RxdataMPA1((64*(i)-64)+(1:64));
    X_dataB(i,1:N_sample)=RxdataMPB1((64*(i)-64)+(1:64));
end
% MA5=fitrlinear(X_dataA,a(1:Trainsize));
% MB5=fitrlinear(X_dataB,b(1:Trainsize));
MA1=fitrlinear(X_dataA,a(1:Trainsize));
MB1=fitrlinear(X_dataB,b(1:Trainsize));

% % MA5=fitrtree(X_dataA,a(1:Trainsize));
% % MB5=fitrtree(X_dataB,b(1:Trainsize));
% % net=fitnet(20,'trainlm');
% % net.trainParam.max_fail=1000;
% % net.trainParam.epochs=5000;
% % netA=train(net,X_dataA',a(1:Trainsize)');
% % netB=train(net,X_dataB',b(1:Trainsize)');
% % RxdataA52=netA(X_dataAC')';
% % 
% % RxdataB52=netB(X_dataBC')';
% 
 mod2 = comm.PAMModulator('ModulationOrder',4);%specify the object
% 
% % RxdataA52=predict(MA5,X_dataAC);
% % 
% % RxdataB52=predict(MB5,X_dataBC);
% RxdataA152=predict(MA5,X_dataAC15);
% 
% RxdataB152=predict(MB5,X_dataBC15);
 RxdataAM2=predict(MA1,X_dataAC15);
% 
 RxdataBM2=predict(MB1,X_dataBC15);
 mod1=comm.PAMDemodulator;
% % RxdataB52=real(step(mod2,step(mod1,RxdataB52)));
% % RxdataA52=real(step(mod2,step(mod1,RxdataA52)));
% RxdataB152=real(step(mod2,step(mod1,RxdataB152)));
% RxdataA152=real(step(mod2,step(mod1,RxdataA152)));
 RxdataBM2=real(step(mod2,step(mod1,RxdataBM2)));
 RxdataAM2=real(step(mod2,step(mod1,RxdataAM2)));
...
mod2 = comm.PAMModulator('ModulationOrder',4);%specify the object
% RxdataAM2=real(step(mod2,step(mod1,RxdataAM1)));%get the received Inphase signal for the 15dB awgn channel case
% RxdataBM2=real(step(mod2,step(mod1,RxdataBM1)));%get the received quadrature signal for the 15dB awgn channel case
...
bitreceive=qam16recombine(RxdataAM2,RxdataBM2);%perform the parallel to serial transform and map the signal to binary
